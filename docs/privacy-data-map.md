...
| `kreiz_contact_requests` | `payload` JSONB : nom, email, sujet, message, consentement (champs du formulaire déclaré) ; statut de traitement ; état de notification (`pending`/`failed`/`sent`, tentatives, `notification_failure` JSON, `notification_claimed_at` — horodatage de bail anti double-envoi, pas une donnée personne) | **oui** (nom, email, message) | formulaire public | boîte de contact admin + notifications | rétention **opt-in** (passe de fermeture) : `KREIZ_CONTACT_RETENTION_DAYS` (30–730 j, reco 180) purge par lots les demandes **traitées** (`handled`) plus anciennes que N jours, via `POST /api/maintenance` (cron) ; sans la variable, illimitée — reste à la charge du Project (voir [technical-debt.md](technical-debt.md)) | statut `handled` (UI) ; les demandes `new` ne sont **jamais** purgées automatiquement | privée |
...
| `kreiz_rate_limits` | clé HMAC (IP/email pseudonymisées — IPv6 réduite à son /64 depuis la revue finale), compteur fenêtré | non (pseudonymisée par HMAC avec `KREIZ_SECRET`) | guards (login, formulaires, analytics) | anti-abus | fenêtres glissantes (15 min login, 10 min contact, 30/min analytics) ; purge opportuniste **bornée par lots**, désormais aussi exécutée par la rétention analytics | automatique (expiration) + purge | privée |
